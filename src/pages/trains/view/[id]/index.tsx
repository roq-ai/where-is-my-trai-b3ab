...
import { getTrainById } from 'apiSdk/trains';
import { TrainInterface } from 'interfaces/train';
import { SearchListPage } from 'pages/searches';

function TrainViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<TrainInterface>(
    () => (id ? `/trains/${id}` : null),
    () =>
      getTrainById(id, {
        relations: [],
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                <FormListItem label="Current Location" text={data?.current_location} />
